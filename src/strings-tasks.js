...
function isString(value) {
  if (typeof value === 'string' || toString.call(value) === '[object String]')
    return true;
  return false;
}
...
function formatTime(minutes, seconds) {
  ...
  const secToString = seconds.toString();
  const min = minToString.padStart(2, 0);
  const sec = secToString.padStart(2, 0);
  return `${min}:${sec}`;
}
...
function reverseString(str) {
  ...
  const strReverst = arrRevers.join('');
  return strReverst;
}
...
function orderAlphabetically(str) {
  ...
  const arrSort = strToArr.sort();
  const arrToString = arrSort.join('');
  return arrToString;
}
...
function reverseWords(str) {
  const arr = str.split(' ');
  const reversedArr = arr.map((qq) => qq.split('').reverse().join(''));
...
function invertCase(str) {
  ...
  const arrInvertToString = arr.join('');
  return arrInvertToString;
}
...
function encodeToRot13(str) {
  ...
  const Cezar = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str.replace(/[A-Z]/gi, (char) => alphabet[Cezar.indexOf(char)]);
}
